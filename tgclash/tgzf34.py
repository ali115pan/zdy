...
recent_days = config.get('recent_days', 0)  # 新增参数，默认值为 0
...
class TGForwarder:
    def __init__(self, api_id, api_hash, string_session, channels_groups_monitor, forward_to_channel,
                 limit, replies_limit, include, exclude, check_replies, proxy, checknum, replacements, message_md, channel_match, hyperlink_text, past_years, only_today, recent_days):
        self.urls_kw = ['magnet', 'drive.uc.cn', 'caiyun.139.com', 'cloud.189.cn', 'pan.quark.cn', '115cdn.com','115.com', 'anxia.com', 'alipan.com', 'aliyundrive.com','pan.baidu.com','mypikpak.com','123684.com', '123952.com', '123912.com', '123865.com','123pan.com','123952.cn']
        ...
        self.recent_days = recent_days  # 新增参数
    ...
    async def del_channel_forward_count_msg(self):
        ...
        if not chat_forward_count_msg_id:
            logger.info("没有需要删除的消息记录。")
            return

        # 删除 forward_to_channel 的消息
        forward_to_channel_message_id = chat_forward_count_msg_id.get(self.forward_to_channel)
        if forward_to_channel_message_id is not None:
            try:
                await self.client.delete_messages(self.forward_to_channel, [forward_to_channel_message_id])
                logger.info(f"已删除 forward_to_channel 的消息: {forward_to_channel_message_id}")
            except Exception as e:
                logger.error(f"删除 forward_to_channel 的消息失败: {e}")

        # 删除 channel_match 中目标频道的消息
        if self.channel_match:
            for rule in self.channel_match:
                target_channel_msg_id = chat_forward_count_msg_id.get(rule['target'])
                if target_channel_msg_id is not None:
                    try:
                        await self.client.delete_messages(rule['target'], [target_channel_msg_id])
                        logger.info(f"已删除 {rule['target']} 的消息: {target_channel_msg_id}")
                    except Exception as e:
                        logger.error(f"删除 {rule['target']} 的消息失败: {e}")
    ...
    async def send_daily_forwarded_count(self):
        ...
        try:
            ...
            logger.warning(f"触发 FloodWaitError，需要等待 {e.seconds} 秒")
            await asyncio.sleep(e.seconds)  # 等待指定时间
            await self.client.pin_message(self.forward_to_channel, sent_message.id)  # 重试
        ...
        if self.channel_match:
            for rule in self.channel_match:
                ...
                try:
                    ...
                    logger.warning(f"触发 FloodWaitError，需要等待 {e.seconds} 秒")
                    await asyncio.sleep(e.seconds)  # 等待指定时间
                    await self.client.pin_message(rule['target'], sm.id)  # 重试
    ...
    def categorize_urls(self, urls):
        categories = {
            ...
            "pikpak": ["mypikpak.com"],
            "baidu": ["pan.baidu.com"],
            "123pan": ["123684.com", "123952.com", "123912.com", "123865.com","123pan.com","123952.cn"],
            "others": []
        }
            ...
            try:
                ...
                async for message in self.client.iter_messages(chat, limit=1000):
                    if message.message:
                        ...
                        # 处理“提取码:”前面没有 ? 的情况
                        if "提取码:" in link and "?提取码:" not in link:
                            link = link.replace("提取码:", "?提取码:")
                        # 去掉“提取码:”部分，但保留提取码的值
                        link = re.sub(r'提取码[:：]\s*', '', link).strip()
    ...
    async def forward_messages(self, chat_name, limit, hlinks, hsizes):
        ...
        try:
            ...
            async for message in self.reverse_async_iter(messages, limit=limit):
                if self.recent_days > 0:  # 如果 recent_days 大于 0，则过滤消息
                    message_china_time = message.date + self.china_timezone_offset
                    if message_china_time.date() < (datetime.utcnow() + self.china_timezone_offset - timedelta(days=self.recent_days)).date():
                        continue
                                ...
                                for category, keywords in self.hyperlink_text.items():
                                    ...
                                    for keyword in keywords:
                                        if keyword in text:
                                            text = text.replace(keyword, url)
                        # 处理“提取码:”前面没有 ? 的情况
                        if "提取码:" in text and "?提取码:" not in text:
                            text = text.replace("提取码:", "?提取码:")
                        # 去除“提取码:”部分，但保留提取码的值
                        text = re.sub(r'提取码[:：]\s*', '', text)
    ...
    async def main(self):
        ...
        for chat_name in valid_channels:
            ...
            if '|' in chat_name:
                ...
                chat_name = channel_name.split('|')[0]
            global total
            total = 0
...
TGForwarder(api_id, api_hash, string_session, channels_groups_monitor, forward_to_channel, limit, replies_limit,
            include, exclude, check_replies, proxy, checknum, replacements, message_md, channel_match, hyperlink_text, past_years, only_today, recent_days).run()
