...
def delete_invalid_records(conn, vod_ids, vod_pics):
    ...
    try:
        ...
        # 检查数据库连接是否有效
        if not check_database_connection(conn):
            logger.error("数据库连接已失效，无法执行删除操作")
            return
...
async def main():
    # 清空 invalid_records.json 文件
    if os.path.exists(INVALID_JSON_PATH):
        with open(INVALID_JSON_PATH, 'w', encoding='utf-8') as file:
            file.write("[]")
        logger.info(f"已清空文件: {INVALID_JSON_PATH}")
    ...
    # 删除数据库中无效的记录和对应的图片文件
    if invalid_vod_ids:
        # 检查数据库连接是否有效，如果失效则重新连接
        if not check_database_connection(conn):
            logger.info("数据库连接已失效，尝试重新连接...")
            conn = connect_to_database(config)
            if not conn:
                logger.error("重新连接数据库失败，无法执行删除操作")
                return
