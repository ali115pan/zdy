...
class LinkValidator:
    ...
    async def validate_links(self):
        ...
        # 分类有效和无效数据
        first_pass_valid_data = []
        first_pass_invalid_data = []

        for item, is_valid in zip(data, results):
            if is_valid:
                first_pass_valid_data.append(item)
            else:
                first_pass_invalid_data.append(item)

        # 第二次校验（仅对第一次校验失效的数据进行）
        second_pass_tasks = [limited_check(item) for item in first_pass_invalid_data]
        second_pass_results = await asyncio.gather(*second_pass_tasks)

        # 最终分类
        for item, is_valid in zip(first_pass_invalid_data, second_pass_results):
            if is_valid:
                first_pass_valid_data.append(item)  # 第二次校验通过，重新归类为有效
            else:
                # 如果链接失效，删除对应的图片文件
        ...
        # 保存有效和无效的数据
        await self.save_json_data(first_pass_valid_data, self.json_path)
